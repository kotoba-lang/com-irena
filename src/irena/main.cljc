(ns irena.main
  "Kotodama WASM entrypoint for the irena clean-room actor (L4) — Clojure port.
  ...
  facts (`irena.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Meter"       :plural "meters"        :id-prefix "irena_met"
    :fields [:serial :type :location]            :required [:serial :type]
    :coerce {}                       :refs {}}
   {:entity "Reading"     :plural "readings"      :id-prefix "irena_rea"
    :fields [:meterId :value :unit :readAt]      :required [:value :unit]
    :coerce {:value :float}          :refs {:meterId "Meter"}}
   {:entity "Node"        :plural "nodes"         :id-prefix "irena_nod"
    :fields [:name :region :voltageKv]           :required [:name :region]
    :coerce {:voltageKv :float}      :refs {}}
   {:entity "LoadForecast" :plural "loadforecasts" :id-prefix "irena_loa"
    :fields [:nodeId :period :megawatts]         :required [:period :megawatts]
    :coerce {:megawatts :float}      :refs {:nodeId "Node"}}
   {:entity "MarketBid"   :plural "marketbids"    :id-prefix "irena_mar"
    :fields [:nodeId :price :megawatts :interval] :required [:price :megawatts]
    :coerce {:price :float :megawatts :float} :refs {:nodeId "Node"}}
   {:entity "Tag"         :plural "tags"          :id-prefix "irena_tag"
    :fields [:nodeId :address :value :quality]   :required [:address :value]
    :coerce {:value :float}          :refs {:nodeId "Node"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"irena.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "irena-compat" :tier tier :entities entities} 200])
